...
export const addProduct = async (req, res) => {
	try {
		...
		const quantity = variants.reduce(
			(acc, variant) => acc + variant.size.stock,
			0
		);
...
export const getProductSkuByCode = async (req, res) => {
	try {
		const code = req.params.code;
		if (code === undefined) {
			return res.status(403).json({ message: "code is required" });
		}

		const product = await Product.findOne({ "variants.sku": code });
		if (product === null) {
			return res.status(404).json({ message: "Product not found" });
		}

		const matchedVariant = product.variants.find(
			(variant) => variant.sku === code
		);
		if (!matchedVariant) {
			return res.status(404).json({ message: "Variant not found" });
		}
		const image = product.images.find(
			(i) => i.image_id === matchedVariant.image_id
		);
		const responseData = {
